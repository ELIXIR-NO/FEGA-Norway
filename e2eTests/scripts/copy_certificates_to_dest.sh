...
mkdir -p /volumes/proxy-certs/ssl/ /volumes/proxy-certs/jwt/ /volumes/proxy-certs/store/ &&
  ...
  cp server_with_ca.p12 /volumes/proxy-certs/ssl/server_with_ca.p12 &&
  cp truststore.p12 /volumes/proxy-certs/ssl/truststore.p12 &&
  ...
  #cp keystore.jks /volumes/proxy-certs/store/keystore.jks
